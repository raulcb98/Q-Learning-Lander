...
public class VectorQLearning {
	 ...
	 */
	public VectorQLearning(QTable qTable, QTable visitedStates) {
		this.qTable = qTable;
	 ...
	 * @return next action.
	 */
	public ACTIONS learn(ScalarAgentState previousState, ACTIONS lastAction, ScalarAgentState currentState) {
	 ...
	 * @return reward.
	 */
	private float reward(ScalarAgentState previousState, ACTIONS lastAction, ScalarAgentState currentState) {
		...
		// Distance reward
		float previousDistanceAxisX = previousState.distanceToPortal(ScalarAgentState.AXISY);
		float distanceAxisX = currentState.distanceToPortal(ScalarAgentState.AXISX);
		int signo = 1;
//		if(distanceAxisX > previousDistanceAxisX) signo = -1;
		...
		if(currentState.isAgentOverPortal()) {
			float distanceAxisY = currentState.distanceToPortal(ScalarAgentState.AXISY);
...
//		finalReward += signo*BIGREWARD*currentWallDistance;
	 ...
	 * @return next action.
	 */
	private ACTIONS nextAction(ScalarAgentState currentState) {
