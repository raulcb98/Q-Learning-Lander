 ...
 *
 */
public class ScalarAgentState extends ScalarState {
	 ...
	 */
	public ScalarAgentState(StateObservation stateObs) {
		orientationRad = 0;
	 ...
	 */
	public ScalarAgentState(ScalarAgentState obj) {
		super(obj);
	...
	public void perceive(StateObservation stateObs) {
		...
		for(int i = 0; i < ScalarState.NUMATTRIBUTES; i++) {
			arrayStateValues.add(0);
		}
		
		// Perceive orientation
		updateOrientation(stateObs);
		arrayStateValues.set(ScalarState.POSORIENTATION, perceiveOrientation());
		
		// Perceive displacement
		arrayStateValues.set(ScalarState.POSDISPLACEMENT, perceiveDisplacement(stateObs));
		
		// Perceive compass
		int compassValue = perceiveCompass();
//		compassValue = correctCompass(stateObs, compassValue);
		arrayStateValues.set(ScalarState.POSCOMPASS, compassValue);
		
		// Perceive fast
		arrayStateValues.set(ScalarState.POSFAST, perceiveFast(stateObs));
	...
	private int degreesToRegion(float degrees, int axis) {
		...
		// Check region
		if(axis == UPPER) {
			if(degrees < iniRightGreenZone || degrees > downValue) return ScalarState.RIGHTREDZONE;
			if(degrees > finLeftGreenZone) return ScalarState.LEFTREDZONE;
		} else {
			if(degrees > finRightGreenZone || degrees < upperValue) return ScalarState.RIGHTREDZONE;
			if(degrees < iniLeftGreenZone) return ScalarState.LEFTREDZONE;
		}
		
		if(finRightGreenZone   >= degrees && degrees >= iniRightGreenZone)   return ScalarState.RIGHTGREENZONE;
		if(finCentralGreenZone >= degrees && degrees >= iniCentralGreenZone) return ScalarState.CENTRALGREENZONE;
		if(finLeftGreenZone    >= degrees && degrees >= iniLeftGreenZone)    return ScalarState.LEFTGREENZONE;
	...
	private int perceiveCompass() {
		if(this.portalCellPos == null) return ScalarState.SOUTH;
		
		if(overPosition(agentCellPos, portalCellPos)) return ScalarState.SOUTH;
		if(underPosition(agentCellPos, portalCellPos)) return ScalarState.NORTH;
		if(leftPosition(agentCellPos, portalCellPos)) return ScalarState.EAST;
		if(rightPosition(agentCellPos, portalCellPos)) return ScalarState.WEST;
		
		return ScalarState.SOUTH;
	}
	...
	private int correctCompass(StateObservation stateObs, int currentCompass) {
		if(currentCompass == ScalarState.WEST || currentCompass == ScalarState.EAST) {
	 ...
	 */
	public static int obeyCompass(ScalarAgentState previousState, ScalarAgentState currentState, int previousCompass) {
		...
		switch (previousCompass) {
			case ScalarState.NORTH:
				if(dify == 0) return ScalarState.NONE;
				if(dify < 0) return ScalarState.TRUE;
				if(dify > 0) return ScalarState.FALSE;
			case ScalarState.SOUTH:
				if(dify == 0) return ScalarState.NONE;
				if(dify < 0) return ScalarState.FALSE;
				if(dify > 0) return ScalarState.TRUE;
			case ScalarState.WEST:
				if(difx == 0) return ScalarState.NONE;
				if(difx < 0) return ScalarState.TRUE;
				if(difx > 0) return ScalarState.FALSE;
			case ScalarState.EAST:
				if(difx == 0) return ScalarState.NONE;
				if(difx < 0) return ScalarState.FALSE;
				if(difx > 0) return ScalarState.TRUE;
			
		}
		return ScalarState.ERROR;
	}
	...
	private int perceiveFast(StateObservation stateObs) {
		return (stateObs.getAvatarSpeed() > speed_limit ? ScalarState.TRUE : ScalarState.FALSE);
	}
	...
	public boolean isOrientationInGreenZone() {
		return orientation == ScalarState.CENTRALGREENZONE ||
			   orientation == ScalarState.LEFTGREENZONE ||
			   orientation == ScalarState.RIGHTGREENZONE;
	}
	...
	public boolean isDisplacementInGreenZone() {
		return displacement == ScalarState.CENTRALGREENZONE || 
			   displacement == ScalarState.LEFTGREENZONE ||
			   displacement == ScalarState.RIGHTGREENZONE;
	}
